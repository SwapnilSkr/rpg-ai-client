...
class _PlayViewState extends State<_PlayView> {
  ...
  /// "This person is my sister"). Opens a sheet pre-filled with the tapped
  /// name, an optional role, and an optional kinship relation. On save the
  /// cubit calls the server's `/chronicle/track/:instanceId` endpoint, which
  /// mints/updates the codex card, promotes any scene-participant / kinship
  /// stub entity, writes the typed edge, and ledgers the delta on the latest
  /// event so a rewind replays it.
  void _showTrackEntitySheet(
    BuildContext context,
    PlayCubit cubit,
    String name,
  ) {
...
class _TrackEntitySheet extends StatefulWidget {
  final String name;
  ...
  @override
  State<_TrackEntitySheet> createState() => _TrackEntitySheetState();
}

class _TrackEntitySheetState extends State<_TrackEntitySheet> {
  late final TextEditingController _name;
  late final TextEditingController _role;
  ...
  static const _relationOptions = <(String, String)>[
    ('', 'No relation'),
    ('sibling_of', 'Sibling (sister/brother)'),
    ('parent_of', 'Parent (mother/father)'),
    ('child_of', 'Child (son/daughter)'),
    ('partner_of', 'Partner (wife/husband)'),
    ('superior_of', 'Superior (liege/mentor)'),
    ('subordinate_of', 'Subordinate (vassal/student)'),
    ('kin_of', 'Kin (other family)'),
    ('bonded_of', 'Bonded (sworn/oathmate)'),
  ];
  ...
  void initState() {
    ...
    _relationLabel = TextEditingController();
  }
  ...
  Future<void> _track() async {
    if (_saving) return;
    final trackName = _name.text.trim();
